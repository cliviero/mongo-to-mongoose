...
async function generateSchemaFromMongo(connectionUrl, collectionName, dbName, copyToClipboard, typeKey, sampleSize) {
  try {
    ...
    const schemaName = `${capitalize(collectionName)}Schema`
    const typeKeyOption = typeKey ? `, { typeKey: '${typeKey}' }` : '';
    const schema = `import mongoose from 'mongoose';\n\n` +
      `export const ${schemaName} = new mongoose.Schema(${flatMapToMongooseJSONSchema(flatMap, 2, typeKey)}${typeKeyOption});\n\n` +
      `export const ${capitalize(collectionName)}Model = mongoose.model('${collectionName}', ${schemaName}, '${collectionName}');`;
